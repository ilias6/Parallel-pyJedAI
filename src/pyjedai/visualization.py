...
#     experiments = experiments.groupby(['weighting_scheme', 'algorithm'])
...
#         plt.plot(budgets, average_feature_values, label=str(group_name), marker='o', linestyle='-')

#     # Customize the plot
#     plt.xlabel('Budget')
#     plt.ylabel(f'Average {feature.capitalize()}')
#     plt.legend(title=attributes, fontsize="7", loc='lower right')
#     plt.title(f'{dataset_name.capitalize()} - Average {feature.capitalize()} vs. Budget Curves')
#     plt.grid(grid)
